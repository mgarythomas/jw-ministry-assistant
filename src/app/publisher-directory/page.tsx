...
import {
  Menu, Bell, HelpCircle, Search, UserPlus,
  ...
  return (
    <div className="bg-surface text-on-surface min-h-screen flex overflow-hidden font-body">
      ...
      <div className="flex-1 flex flex-col h-screen relative">
        {/* TopNavBar Component */}
        <header className="fixed top-0 w-full z-50 bg-[#fbf9f8]/70 dark:bg-slate-950/70 backdrop-blur-md flex justify-between items-center px-6 py-4 md:pl-8 lg:ml-64 lg:w-[calc(100%-16rem)]">
          <div className="flex items-center gap-4">
            <button className="md:hidden text-on-surface">
              <Menu size={24} />
            </button>
            <span className="text-2xl font-bold text-[#1b1c1c] dark:text-[#fbf9f8] font-headline md:hidden">Elder</span>
          </div>
          <div className="flex items-center gap-6 md:w-full md:justify-end">
            <div className="hidden lg:flex items-center bg-surface-container rounded-full px-4 py-2 gap-2 w-80">
              <Search size={20} className="text-on-surface-variant" />
              <input className="bg-transparent border-none outline-none text-sm w-full focus:ring-0" placeholder="Search records..." type="text" />
            </div>
            <div className="flex items-center gap-4">
              <button className="text-[#1b1c1c] dark:text-[#efeded] hover:bg-[#efeded] dark:hover:bg-slate-800 transition-colors p-2 rounded-full">
                <Bell size={24} />
              </button>
              <button className="text-[#1b1c1c] dark:text-[#efeded] hover:bg-[#efeded] dark:hover:bg-slate-800 transition-colors p-2 rounded-full">
                <HelpCircle size={24} />
              </button>
              <Avatar className="w-8 h-8 rounded-full md:hidden">
                <AvatarImage src="https://lh3.googleusercontent.com/aida-public/AB6AXuBuf5R8rKjB9WQohxM0Xjj33eOM39PcfkXG57NHPnM0Y87rb663O-bQQhc3Q_CGNNSJx_mT5LirHdYEIJB4tRTquikVtd7iUkSbEs7-6sy6pV691OwrEmeGrX9cbLm7rjiRADsU34P5KUlWdfN3S1Ykay7_vFNp7iCOP9GB7AjuhaZrpUSxWxzz652TCmRZ28GgzcEL6v_KrAL8rWx4mnHOj9yhTqTaw-PkBDAb9vOupvvTKcqInQnbCNcS14rP-fHo4ZKG1eDz" alt="Admin" className="object-cover" />
                <AvatarFallback>SA</AvatarFallback>
              </Avatar>
            </div>
          </div>
