...
import {
  ...
  Bell, Settings, Navigation, ZoomIn, ZoomOut, MapPin, LocateFixed
} from "lucide-react";
  ...
  return (
    <div className="bg-background text-on-background selection:bg-primary-container/30 h-screen font-body flex overflow-hidden">
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        {/* TopAppBar */}
        <header className="fixed top-0 w-full z-50 bg-white/70 dark:bg-stone-900/70 backdrop-blur-md shadow-sm dark:shadow-none flex justify-between items-center px-6 h-16 md:static md:bg-transparent md:backdrop-blur-none border-none">
          <div className="flex items-center gap-4 flex-1">
            <h1 className="text-xl font-bold text-stone-900 dark:text-stone-50 hidden md:block font-headline">Territory Overview</h1>
            <div className="relative w-full max-w-xl group">
              <Search size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-stone-400 group-focus-within:text-primary transition-colors" />
              <input className="w-full bg-surface-container-low border-none rounded-full py-2.5 pl-12 pr-4 text-sm focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-stone-400 outline-none" placeholder="Search by map no, suburb or street" type="text" />
            </div>
          </div>
          <div className="flex items-center gap-2 ml-4">
            <button className="p-2 text-stone-500 hover:bg-stone-100 dark:hover:bg-stone-800 rounded-full transition-colors relative">
              <Bell size={24} />
