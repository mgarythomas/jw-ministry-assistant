...
import { BookOpen, Printer as Print, Upload, User, UserCircle, Users, Settings, MoreHorizontal, Lightbulb, UserPlus, Search, Menu, Bell, Calendar, Plus } from "lucide-react";

export default function MeetingScheduler() {
  return (
    <div className="bg-surface text-on-surface min-h-screen">
      {/* TopNavBar */}
      <header className="fixed top-0 w-full z-50 bg-surface/70 dark:bg-zinc-900/70 backdrop-blur-md flex justify-between items-center px-6 py-3 lg:ml-64 lg:w-[calc(100%-16rem)] shadow-sm">
        <div className="flex items-center gap-4">
          <button className="lg:hidden p-2 text-zinc-600">
            <Menu size={24} />
          </button>
          <h1 className="text-xl font-bold text-blue-800 dark:text-blue-200 font-headline tracking-tight">Congregation Manager</h1>
        </div>
        <div className="flex items-center gap-3 md:gap-6">
          <div className="relative hidden sm:flex items-center">
            <Search size={20} className="absolute left-3 text-zinc-400" />
            <input className="pl-10 pr-4 py-2 bg-surface-container-low border-none rounded-full text-sm focus:ring-2 focus:ring-primary w-48 xl:w-64 outline-none" placeholder="Search records..." type="text" />
          </div>
          <div className="flex items-center gap-1 md:gap-4">
            <button className="text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 p-2 rounded-full transition-colors">
              <Bell size={24} />
            </button>
            <button className="text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 p-2 rounded-full transition-colors hidden sm:block">
              <Settings size={24} />
            </button>
            <Avatar className="w-8 h-8 rounded-full border border-surface-variant ml-1">
              <AvatarImage src="https://lh3.googleusercontent.com/aida-public/AB6AXuAGiLw5R5imG5YVVJiMS3p7hS3Ogr4wlWgN-JVXo4IlgBgHOTs1DpaaksQA7Lb39a6uC5VA3q4TV_zsvmBzIP38dMV_mH3djv2de40QFiYJ63Y30Qhac-OUKvoYSb7DgZvN7RSfKHs08BiiAZ-GkUH93gbW5zeUEvJ8EC65kBLEryBn4MvB_AlnhQT7OEAd2A2H-Zw2D4bHzjoLwu-H31INSXysODTozwtDITbxJbu_Dxty6u-4PzjB7j0d6VWczOY0wwO1R5q9" alt="Elder" className="object-cover" />
        ...
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex flex-wrap gap-3">
              <Button variant="secondary" className="flex-1 md:flex-none px-6 py-6 bg-surface-container-high rounded-xl font-semibold text-on-surface hover:bg-surface-variant transition-colors flex items-center justify-center gap-2 touch-manipulation border-none shadow-none">
                <Print size={20} /> Print
              </Button>
              <Button className="flex-1 md:flex-none px-6 py-6 bg-primary text-white rounded-xl font-semibold hover:opacity-90 transition-opacity flex items-center justify-center gap-2 shadow-lg touch-manipulation border-none">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
            ...
            <div className="xl:col-span-4 space-y-8 sticky xl:top-24">
              ...
              <Card className="bg-surface-container-lowest p-1 rounded-3xl shadow-lg overflow-hidden border border-zinc-100">
                <CardContent className="p-6">
                  ...
                  <div className="aspect-[3/4] rounded-2xl bg-zinc-100 overflow-hidden relative group">
                    ...
                    <div className="absolute inset-0 bg-black/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="secondary" className="bg-white hover:bg-zinc-100 px-5 py-6 rounded-full font-bold text-xs shadow-xl flex items-center gap-2 touch-manipulation border-none">
                        View Large
                      </Button>
                    </div>
                  </div>
